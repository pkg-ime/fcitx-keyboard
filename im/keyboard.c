...
static void enchantDictDescribeFn(const char * const lang_tag,
                                  ...
                                  void * user_data);
static char*  FcitxKeyboardCheckEnchantSupport(FcitxKeyboard* keyboard, const char* langCode);
static boolean LoadKeyboardConfig(FcitxKeyboardConfig* fs);
...
static void enchantDictDescribeFn(const char * const lang_tag,
                                  ...
                                  void * user_data)
{
    FcitxKeyboard* keyboard = user_data;
    
    utarray_push_back(keyboard->enchantLanguages, &lang_tag);
}

char* FcitxKeyboardCheckEnchantSupport(FcitxKeyboard* keyboard, const char* langCode)
{
    if (!langCode)
        return NULL;
    ...
    for (i = 0; i < utarray_len(keyboard->enchantLanguages); i++) {
        char** lang = (char**) utarray_eltptr(keyboard->enchantLanguages, i);
        int len = strlen(*lang);
        if (len > bestlen && strncmp(*lang, langCode, len) == 0) {
            best = i;
            bestlen = len;
        }
    }
    if (best >= 0)
        return *(char**) utarray_eltptr(keyboard->enchantLanguages, best);
    return NULL;
}
...
void* FcitxKeyboardCreate(FcitxInstance* instance)
{
    ...
    bindtextdomain("xkeyboard-config", LOCALEDIR);
    if (!LoadKeyboardConfig(&keyboard->config))
    {
        free(keyboard);
        return NULL;
    }
    ...
    enchant_broker_list_dicts(keyboard->broker, enchantDictDescribeFn, keyboard);
    ...
    if (rules) {
        ...
        FcitxKeyboardLayoutCreate(keyboard, description, "en", "en", NULL);
        free(description);
    }
...
INPUT_RETURN_VALUE FcitxKeyboardDoInput(void *arg, FcitxKeySym sym, unsigned int state)
{
    ...
    if (layout->owner->config.bEnableWordHint && strcmp(keyboard->dictLang, currentLang) != 0) {
        ...
        if (FcitxKeyboardCheckEnchantSupport(keyboard, keyboard->dictLang))
            keyboard->dict = enchant_broker_request_dict(keyboard->broker, keyboard->dictLang);
    }
    
    if (keyboard->dict && layout->owner->config.bEnableWordHint) {
        ...
        {
            size_t len = strlen(layout->buffer);
            if (len != 0) {
                enchant_dict_add_to_personal(keyboard->dict, layout->buffer, len);
                return IRV_DO_NOTHING;
            }
        }
        ...
        if (IsValidChar(result) || FcitxHotkeyIsHotKeySimple(sym, state) || IsValidSym(sym, state))
        {
            if (IsValidChar(result) || FcitxHotkeyIsHotKeyLAZ(sym, state) || FcitxHotkeyIsHotKeyUAZ(sym, state) || IsValidSym(sym, state)) {
                ...
                if (strlen(layout->buffer) >= FCITX_KEYBOARD_MAX_BUFFER) {
                    FcitxInstanceCommitString(instance, FcitxInstanceGetCurrentIC(instance), layout->buffer);
                    layout->cursorPos = 0;
                    layout->buffer[0] = '\0';
                }
                size_t len = strlen(layout->buffer);
                if (layout->buffer[layout->cursorPos] != 0)
                {
                    memmove(layout->buffer + layout->cursorPos + charlen, layout->buffer + layout->cursorPos, len - layout->cursorPos);
                }
                layout->buffer[len + charlen] = 0;
                strncpy(&layout->buffer[layout->cursorPos], buf, charlen);
                layout->cursorPos += charlen;
            ...
            {
                size_t slen = strlen(layout->buffer);
                if (slen > 0)
                {
                    if (layout->cursorPos > 0) {
                        size_t len = fcitx_utf8_strlen(layout->buffer);
                        char* pos = fcitx_utf8_get_nth_char(layout->buffer, len - 1);
                        layout->cursorPos = pos - layout->buffer;
                        memset(layout->buffer + layout->cursorPos, 0, sizeof(char) * (slen - layout->cursorPos));
                        return IRV_DISPLAY_CANDWORDS;
                    }
                    else
                        return IRV_DO_NOTHING;
        ...
        if (strlen(layout->buffer) > 0) {
            ...
            strcpy(FcitxInputStateGetOutputString(input), layout->buffer);
...
INPUT_RETURN_VALUE FcitxKeyboardGetCandWords(void* arg)
{
    ...
    if (layout->buffer[0] == '\0')
        return IRV_CLEAN;
    FcitxCandidateWordSetPageSize(FcitxInputStateGetCandidateList(input), config->iMaxCandWord);
    FcitxCandidateWordSetChoose(FcitxInputStateGetCandidateList(input), DIGIT_STR_CHOOSE);
    size_t bufferlen = strlen(layout->buffer);
    strcpy(FcitxInputStateGetRawInputBuffer(input), layout->buffer);
    FcitxInputStateSetRawInputBufferSize(input, bufferlen);
    FcitxMessagesAddMessageAtLast(FcitxInputStateGetClientPreedit(input), MSG_INPUT, "%s", layout->buffer);
    FcitxMessagesAddMessageAtLast(FcitxInputStateGetPreedit(input), MSG_INPUT, "%s", layout->buffer);
    FcitxInputStateSetClientCursorPos(input, layout->cursorPos);
    FcitxInputStateSetCursorPos(input, layout->cursorPos);
    
    if (bufferlen < FCITX_KEYBOARD_MIN_HINT_LENGTH || !enchant_dict_check(keyboard->dict, layout->buffer, bufferlen))
        return IRV_DISPLAY_CANDWORDS;
    
    size_t number = 0;
    char **suggestions = enchant_dict_suggest(keyboard->dict, layout->buffer, bufferlen, &number);
    int i;
    for (i = 0 ;i < number && i < config->iMaxCandWord; i ++)
    {
        if (layout->owner->config.bOnlyHintWordWithSamePrefix
            && (strlen(suggestions[i]) <= bufferlen
                || strncmp(suggestions[i], layout->buffer, bufferlen) != 0))
            continue;
...
{
    FcitxKeyboardLayout* layout = (FcitxKeyboardLayout*) arg;
    LoadKeyboardConfig(&layout->owner->config);
}

boolean LoadKeyboardConfig(FcitxKeyboardConfig* fs)
{
    FcitxConfigFileDesc *configDesc = GetKeyboardConfigDesc();
    if (configDesc == NULL)
        return false;

    FILE *fp = FcitxXDGGetFileUserWithPrefix("conf", "fcitx-keyboard.config", "rt", NULL);
...
{
    FcitxConfigFileDesc *configDesc = GetKeyboardConfigDesc();
    FILE *fp = FcitxXDGGetFileUserWithPrefix("conf", "fcitx-keyboard.config", "wt", NULL);
...
processCompose(FcitxKeyboardLayout* layout, uint32_t keyval, uint32_t state)
{
    ...
    layout->composeBuffer[layout->n_compose ++] = keyval;
    layout->composeBuffer[layout->n_compose] = 0;
...
checkCompactTable(FcitxKeyboardLayout* layout, const FcitxComposeTableCompact *table)
{
    ...
    * will overrun val if key is longer than val. */
    if (layout->n_compose > table->max_seq_len)
        return 0;

    seq_index = (const uint32_t *)bsearch(layout->composeBuffer,
    ...
    if (seq_index && layout->n_compose == 1) {
        return INVALID_COMPOSE_RESULT;
    }

    seq = NULL;
    for (i = layout->n_compose - 1; i < table->max_seq_len; i++) {
        row_stride = i + 1;

        if (seq_index[i + 1] - seq_index[i] > 0) {
            seq = (const uint32_t *) bsearch(layout->composeBuffer + 1,
                                            table->data + seq_index[i], (seq_index[i + 1] - seq_index[i]) / row_stride,
                                            sizeof(uint32_t) * row_stride,
                                            compare_seq);
            if (seq) {
                if (i == layout->n_compose - 1)
                    break;
                else {
                    return INVALID_COMPOSE_RESULT;
                }
...
checkAlgorithmically(FcitxKeyboardLayout* layout)
{
    ...
    if (layout->n_compose >= FCITX_MAX_COMPOSE_LEN)
        return 0;

    for (i = 0; i < layout->n_compose && IS_DEAD_KEY(layout->composeBuffer[i]); i++);
    if (i == layout->n_compose)
        return INVALID_COMPOSE_RESULT;

    if (i > 0 && i == layout->n_compose - 1) {
        combination_buffer[0] = FcitxKeySymToUnicode((FcitxKeySym) layout->composeBuffer[i]);
        combination_buffer[layout->n_compose] = 0;
        i--;
        while (i >= 0) {
            switch (layout->composeBuffer[i]) {
...
#undef CASE
            default:
                combination_buffer[i + 1] = FcitxKeySymToUnicode((FcitxKeySym) layout->composeBuffer[i]);
            }
            i--;
        }
        ...
        **/
#if 0
        if (check_normalize_nfc(combination_buffer, layout->n_compose)) {
            gunichar value;
        ...
        i = unorm_normalize(combination_buffer, layout->n_compose, UNORM_NFC, 0, result, FCITX_MAX_COMPOSE_LEN + 1, &state);

        // qDebug () << "combination_buffer = " << QString::fromUtf16(combination_buffer) << "layout->n_compose" << layout->n_compose;
